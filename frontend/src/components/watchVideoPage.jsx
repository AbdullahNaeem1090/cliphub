...
function WatchVideoPage() {
    ...
    if (resp.data.data.subscriber == currUser._id) {
      ...
      dispatch(
        addSubscribedChannel({
          ...
          ContentCreators: [
            {
              _id: currVideo.ownerDetails[0]._id,
              email: currVideo.ownerDetails[0].email,
  ...
  async function newVideoInPL(pl_Name, index) {
    try {
      let resp = await axios.post("/api/playlist/addVideoToPlaylist", {
        pl_Name,
        videoIdPlaylist: currVideo._id,
      });
      if (resp) {
        dispatch(
          addToPlaylist({
  ...
  async function removeVideoFromPlaylist() {
    ...
    let resp = await axios.patch(
      `/api/playlist/removeVideo/${currVideo._id}/${playlistId}/${"private"}`
    );
    if (resp) {
      dispatch(
        remVideoFromPlaylist({
  ...
  return (
    <>
      <div className=" md:p-2 sm:ml-56 scrollbar-custom ">
        <div className="p-3 rounded-lg mt-14 lg:flex overflow-clip w-full">
                ...
                <div className="flex justify-between lg:max-w-[845px] ">
                  <div className="flex items-center">
                    <div>
                      <img
                        src={
                          currVideo?.ownerDetails[0]?.avatar ||
                  ...
                  <div className="flex lg:space-x-6 space-x-2">
                    ...
                    {likes.length > 0 &&
                    likes.some((obj) => obj.likedVideoId === currVideo._id) ? (
                      <button
                        onClick={() =>
                          like(currUser._id, currVideo._id, "removeLike")
                        }
                      >
                        <img
                      ...
                      <button
                        onClick={() =>
                          like(currUser._id, currVideo._id, "addLike")
                        }
                      >
                        <img
                    ...
                    subscribedChannels.some(
                      (obj) =>
                        obj.ContentCreators[0]._id ===
                        currVideo.ownerDetails[0]._id
                    ) ? (
                      <button
                        onClick={unSubscribe}
                        className="text-white rounded-full px-2 h-10 md:font-bold bg-gray-600 ml-5 hover:bg-slate-500"
                      >
                        Subscribed
                      </button>
                    ) : (
                      <button
                        onClick={subscribe}
                        className="text-white rounded-full px-2 h-10 font-bold bg-red-600 ml-5 hover:bg-red-700"
                      >
                        Subscribe
                      </button>
                    )}
                ...
                {currVideo.comments.map((comment, index) => (
                  <div key={comment._id}>
                    ...
                    {currVideo.reply_array.map((obj) =>
                      obj.replies
                        .filter(
                          (reply) => reply.ParentComment_id === comment._id
                        )
                        .map((reply) => (
                          <article
                            key={reply._id}
                            className={`${
                              repButtonId == reply.ParentComment_id &&
                              showRepliess
                                ? ""
                                : "hidden "
          ...
          <div className="flex-1 rounded-lg lg:ml-2 lg:border-l lg:w-[30%] ">
            ...
            <div className="lg:h-[700px] lg:overflow-y-scroll scrollbar-hide lg:border-y ">
              {fetchedVideos
                ...
                lg:h-36 text-white rounded-lg shadow md:flex-row md:w-full hover:bg-white hover:bg-opacity-10 p-1  mb-2 md:p-2 md:mb-0"
                  ...
                  >
                    <img
                      className="object-cover rounded-lg max-h-60 md:w-72 lg:w-44"
                      src={video.thumbnail}
                      alt=""
                    />
                      ...
                      <div className="flex flex-col pl-1 pt-1  leading-normal md:w-full">
                        ...
                        <div className="md:flex items-center">
                          <div className="hidden md:block h-6 w-6 m-1 rounded-full">
                            <img
                              src={
                                video.avatar || "/src/assets/defaultAvatar.png"
                              }
                              alt=""
                              className="object cover w-full h-full rounded-full"
                            />
      ...
      >
        ...
        {playlists &&
          playlists.length > 0 &&
          playlists.map((suggestion, index) => (
            <button
              key={index}
              type="button"
      ...
      <div
        id="comment-noti"
        className="fixed bottom-4 right-4 w-full max-w-xs p-4 text-gray-300 bg-gray-800 rounded-lg shadow-lg hidden"
        role="alert"
      >
